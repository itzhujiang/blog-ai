...
export default function AiDialogue({
  ...
  const renderingMap: Record<AiChatMessageRoleType, (_item: ChatMessage) => ReactElement> = {
    ...
    developer: (item: ChatMessage) => {
      return (
        <div className="flex justify-start gap-3">
          <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-xs font-semibold text-blue-600 shadow-sm dark:text-blue-400">
            DEV
          </div>
          <div className="flex max-w-[88%] flex-col gap-2">
            <div className="rounded-3xl rounded-tl-md border border-blue-500/10 bg-white px-4 py-3 text-sm leading-7 text-text-light shadow-[0_8px_24px_rgba(0,0,0,0.06)] dark:bg-background-dark dark:text-text-dark">
              {item.contentType === 'a2ui' ? <A2UIRenderer value={item.content} /> : item.content}
            </div>
            <span className="px-1 text-[10px] text-text-light/35 dark:text-text-dark/35">{formatDate(item.createdAt)}</span>
          </div>
    ...
    tool: (item: ChatMessage) => {
      return (
        <div className="flex justify-center">
          <div className="inline-flex max-w-[88%] items-center gap-2 rounded-2xl border border-purple-500/15 bg-purple-50/50 px-3 py-2 text-xs text-purple-700 dark:bg-purple-950/30 dark:text-purple-300">
            <span className="material-symbols-outlined text-sm!">construction</span>
            <span>{item.contentType === 'string' ? item.content : 'Tool Result'}</span>
          </div>
        </div>
      );
    },
    reasoning: (item: ChatMessage) => {
      return (
        <div className="flex justify-start gap-3">
          <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-amber-500/10 text-xs font-semibold text-amber-600 shadow-sm dark:text-amber-400">
            <span className="material-symbols-outlined text-sm!">psychology</span>
          </div>
          <div className="flex max-w-[88%] flex-col gap-2">
            <div className="rounded-3xl rounded-tl-md border border-amber-500/10 bg-amber-50/30 px-4 py-3 text-sm leading-7 text-text-light/80 dark:bg-amber-950/20 dark:text-text-dark/80">
              {item.contentType === 'string' ? item.content : 'Reasoning...'}
            </div>
            <span className="px-1 text-[10px] text-text-light/35 dark:text-text-dark/35">{formatDate(item.createdAt)}</span>
          </div>
        </div>
